...
import { Calendar1Icon, ChevronRight, ChevronRightIcon, DollarSignIcon, FileTextIcon, LayoutGridIcon, LogOutIcon, Menu, MenuIcon,SettingsIcon,UserIcon, XIcon } from 'lucide-react'
...
const Sidebar = () => {
    ...
    const sidebarContent = (
        <>
            ...
            <div className='p-3 border-t border-white/6'>
                ...
                hover:bg-rose-500/8 transition-all duration-150' onClick={handleLogout}>
                    <LogOutIcon className='w-[17px] h-[17px]' />
                    <span className='text-[13px] font-medium text-slate-300 ml-2'>Log out</span>
                </button>
            </div>
        </>
    )
  ...
  return (
    <div>
      ...
      <button className='lg:hidden fixed top-4 left-4 z-50 p-2 rounded-md text-gray-700 bg-white 
      shadow-md' onClick={() => setMobileOpen(!mobileOpen)}>
        <MenuIcon size={20} onClick={() => setMobileOpen(!mobileOpen)} />
      </button>
      ...
      {/* Mobile Overlay */}
        ...
        to-slate-950 text-white z-50 flex flex-col transform transition-transform duration-300 ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}>
            {sidebarContent}
        </aside>
    </div>
  )
